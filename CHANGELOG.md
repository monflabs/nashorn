...
`   ` `           ` **The standard libraries, `nashorn-libs`.** A new published artifact (module `org.monflabs.nashorn.libs`) holds what a script expects from its host beyond the language, as `ScriptLibrary` services every engine that sees the jar discovers: the `host` library - WHATWG `setTimeout`, `clearTimeout`, `setInterval`, `clearInterval` on the event loop, `queueMicrotask` on the microtask queue, and forgiving `atob`/`btoa` - and the `fetch` library - `fetch`, `Headers`, `Request` and `Response` over the JDK's `HttpClient`, the promise settling on the script's thread, an HTTP error resolving with `ok` false and a network failure rejecting with a `TypeError`. `--libraries` selects among them. Each is one Java class with a function object per enum entry and a switch, the fetch classes being script over a Java transport; a *Standard Libraries* section documents them, and the playground runs a sample per feature, its `fetch` against public APIs - Open-Meteo's weather and GitHub's.
