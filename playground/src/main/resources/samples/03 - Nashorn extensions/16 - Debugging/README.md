...
1. Tick **Debug in Chrome** on the toolbar. The status line shows the WebSocket URL the server listens on (port 9229).
...
**Pause on next run** stops at the first statement, for a script with no `debugger;` in it.
