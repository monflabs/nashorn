...
- **test262 (ES2024 slice)** — `mvn -Ptest262 verify`, driven by `Test262Runner`, not by `TestFinder`/`ParallelTestRunner`. Read the note below before touching it.
...
## test262 and the ES2024 conformance target
...
`core/pom.xml`) and the ES2024 slice is selected out of it at runtime.

The selector (`Test262Selector`) is a **deny** rule: a test is in scope unless its `features:` name
something that postdates ES2024. That is deliberate — an allow rule keyed on feature tags drops the
