...
- **test262 (ES2023 slice)** — `mvn -Ptest262 verify`, driven by `Test262Runner`, not by `TestFinder`/`ParallelTestRunner`. Read the note below before touching it.
...
## test262 and the ES2023 conformance target
...
`core/pom.xml`) and the ES2023 slice is selected out of it at runtime.

The selector (`Test262Selector`) is a **deny** rule: a test is in scope unless its `features:` name
something that postdates ES2023. That is deliberate — an allow rule keyed on feature tags drops the
...
- Results are diffed against `core/src/test/resources/test262-expectations.txt`, and the run fails on an
  ...
  lists the settled failures — 92 of the 74,600 executions: 8 Annex B indirect-eval "existing var
