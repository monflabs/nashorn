...
for tool in mvn git gh java gpg rsync awk; do
  command -v "$tool" >/dev/null 2>&1 || die "missing required tool: $tool"
done

gh auth status >/dev/null 2>&1 || die "gh is not authenticated (run: gh auth login)"
gpg --list-secret-keys >/dev/null 2>&1 && [ -n "$(gpg --list-secret-keys 2>/dev/null)" ] \
  || die "no GPG secret key found; Central requires signed artifacts"
grep -q '<id>central</id>' "${HOME}/.m2/settings.xml" 2>/dev/null \
  || warn "no <server><id>central</id> found in ~/.m2/settings.xml; the deploy will fail without the Portal token"

branch="$(git rev-parse --abbrev-ref HEAD)"
[ "$branch" = "$RELEASE_BRANCH" ] || die "on branch '$branch', expected '$RELEASE_BRANCH' (set RELEASE_BRANCH to override)"
[ -z "$(git status --porcelain)" ] || die "working tree is not clean; commit or stash first"

note "fetching origin"
git fetch --quiet origin
[ "$(git rev-parse HEAD)" = "$(git rev-parse "origin/$RELEASE_BRANCH")" ] \
  || die "$RELEASE_BRANCH is not in sync with origin/$RELEASE_BRANCH; push or pull first"
...
if [ "${RELEASE_SKIP_GHRELEASE:-}" = "1" ]; then
