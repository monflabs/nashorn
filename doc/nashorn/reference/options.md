...
| `--print-ast`, `--print-lower-ast`, `--print-parse`, `--print-lower-parse`, `--print-symbols` | Print compiler intermediate representations. |
| `--print-no-newline` | Make `print` write without a trailing newline. |
| `--compile-only` | Compile the script and exit without running it. |
| `--profile-callsites`, `--trace-callsites` | Count or trace `invokedynamic` call-site activity. |
| `-xhelp` | Print the complete option list, undocumented ones included. |
