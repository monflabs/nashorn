...
| `@@match` and `@@replace` take the same pristine-regexp guard as `@@split`, and `String.prototype.match`/`replace` are defined in terms of them | `NativeRegExp`, `NativeString` | after any script stores a well-known string symbol: `replace` 82ms → 28ms, `match` 206ms → 119ms per 100-200k | None. The two copies of the algorithm are now one, which also fixes an empty match in unicode mode advancing by a code unit rather than a code point. |
| One matcher per global walk rather than per match, the position in a local rather than in `lastIndex`, an ordinary global replace streamed rather than collected, and a `$`-free replacement appended as it stands | `NativeRegExp` | `matchreplace` -23%; against 15.7, `match(/[a-z]+/g)` 174ms to its 228ms and `replace(re, fn)` 100ms to its 98ms | Only for a global, non-sticky ordinary regexp: a sticky one anchors at `lastIndex`, where the intermediate values are observable, and goes back through `exec`. |
