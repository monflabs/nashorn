...
/**
 * Whether any object anywhere holds a property at an index.
 *
 * Writing to an index an array does not have yet is, by 9.1.9.1, a walk of its
 * prototype chain: an accessor at that index up there is entitled to the write.
 * Walking on every append would cost more than the walk ever finds, because
 * almost no program puts a property at an index of an object that is a
 * prototype - it takes a defineProperty to do it at all, since an ordinary
 * write goes to the array data rather than to the map.
 *
