...
/**
 * ECMA-262 Annex B is gone when --annexB is off - all of it, so that an engine
 * asked for the standard alone is given the standard alone.
 *
 * @test
 * @run
 * @option --annexB=false
...
for (var name in LIBRARY) {
    if (LIBRARY[name]() !== "undefined") {
        fail(name, "undefined", LIBRARY[name]());
    }
}

for (var i = 0; i < SYNTAX.length; i++) {
    if (parses(SYNTAX[i])) {
        fail(JSON.stringify(SYNTAX[i]), "a SyntaxError", "that it parsed");
    }
}

if (callExpressionTarget() !== "SyntaxError") {
    fail("a call expression as an assignment target", "SyntaxError", callExpressionTarget());
}
if (blockFunctionHoisting() !== "undefined") {
    fail("a block-level function declaration", "undefined", blockFunctionHoisting());
}
if (legacyOctalEscape() === "matches") {
    fail("a legacy octal escape", "not to match", legacyOctalEscape());
}
if (quantifiedAssertion() !== "SyntaxError") {
    fail("a quantified assertion", "SyntaxError", quantifiedAssertion());
}
