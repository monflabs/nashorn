...
public abstract class TestHelper {
    ...
    /**
     * Directory the .js test suite lives under, relative to the working
     * directory. Test output is mirrored below {@link #BUILD_ROOT} using the
     * remainder of each test's path, so this prefix is stripped from it.
     */
    public static final String TEST_ROOT   =
        System.getProperty("test.root.dir", "src/test").replace('/', File.separatorChar);
    ...
    protected static File makeBuildDir(final File testFile) {
        ...
        if (!buildDir.isDirectory() && !buildDir.mkdirs()) {
            throw new IllegalArgumentException("could not create build directory " + buildDir
                    + " in " + new File(".").getAbsolutePath());
        }
        return buildDir;
    }
