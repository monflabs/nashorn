...
public final class Test262Host {
    ...
    public static void agentBroadcast(final Object buffer) {
        final java.nio.ByteBuffer storage =
                org.monflabs.nashorn.internal.objects.NativeSharedArrayBuffer.storageOf(buffer);
        if (storage == null) {
    ...
    public static Object agentReceiveBroadcast() {
        ...
        try {
            return org.monflabs.nashorn.internal.objects.NativeSharedArrayBuffer.wrap(
                    inbox.take(), org.monflabs.nashorn.internal.runtime.Context.getGlobal());
        } catch (final InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException(e);
        }
    ...
    private static final String AGENT_HOST_OBJECT =
            ...
            + "    return Java.type('org.monflabs.nashorn.internal.test.framework.Test262Host').evalScript(String(s));"
            + "  },"
            + "  gc: function () {},"
            + "  agent: {"
            + "    receiveBroadcast: function (cb) {"
            + "      cb(Java.type('org.monflabs.nashorn.internal.test.framework.Test262Host').agentReceiveBroadcast());"
            + "    },"
            + "    report: function (r) {"
            + "      Java.type('org.monflabs.nashorn.internal.test.framework.Test262Host').agentReport(String(r));"
            + "    },"
            + "    sleep: function (ms) {"
            + "      Java.type('org.monflabs.nashorn.internal.test.framework.Test262Host').agentSleep(ms);"
            + "    },"
            + "    monotonicNow: function () {"
            + "      return Java.type('org.monflabs.nashorn.internal.test.framework.Test262Host').agentMonotonicNow();"
            + "    },"
            + "    leaving: function () {}"
            + "  }"
            + "};";
